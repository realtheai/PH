#!/usr/bin/env python3
"""
실패한 이미지 재처리 (DB에서 ocr_text가 없는 것들)
"""
import os
import sys
...
# ocr_text가 null이거나 없는 레코드 조회
response = requests.get(
    f"{supabase_url}/rest/v1/phishing_images?select=local_path,image_url&ocr_text=is.null",
    headers=headers,
    timeout=30
)
...
if failed_records:
    ...
    # DB 조회 실패 시 전체 다시 시도
    print(f"📋 전체 이미지 재처리: {len(all_images)}개")
    failed_images = list(all_images.values())
...
print("\n" + "="*100)
print(f"🔄 재처리 시작 ({len(failed_images)}개)")
print("="*100 + "\n")
