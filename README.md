...
2. **벡터 검색 (RAG)**: 1,360개 실제 피싱 사례와 유사도 분석
...
| 출처 | 개수 | 수집 방법 | 데이터 유형 |
|------|------|-----------|-------------|
| **Google Images** | 373개 | 이미지 검색 + OCR | 피싱 문자 스크린샷 |
| **네이버 뉴스** | 687개 | 뉴스 API + 크롤링 | 피싱 사건 기사 |
| **Google News** | 300개 | 뉴스 API | 국내외 피싱 사건 |
| **총계** | **1,360개** | - | 텍스트 + 메타데이터 |
...
1. 크롤링 (scripts/)
   ...
   ├─ Google 뉴스 API (scripts/crawl_international_news.py)
...
# 2. Supabase에서 유사한 사례 검색
similar_cases = supabase.rpc('match_phishing_cases', {
    'query_embedding': embedding,
    'match_threshold': 0.5,  # 코사인 유사도 ≥ 0.5
    'match_count': 5
})
